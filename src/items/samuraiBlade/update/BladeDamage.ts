...
import {
  getHitTargetsInsideArea,
  isHitTargetInsideArea,
} from "../../../helpers/TargetFinding";
import {
  countOccurrencesOfState,
  registerDamageState,
} from "../onDealingDamage/DamageStateHandler";
import { spawnSecretTear } from "../onDealingDamage/SecretTearSpawner";
import {
  handleTearCountSynergies,
  hasSpectral,
} from "../synergy/SynergyHandlers";

const LOG_ID = "BladeDamage";

export function dealSamuraiBladeDamage(
  player: EntityPlayer,
  isCritical: boolean,
): void {
  const targets = getHitTargetsInsideArea(
    player,
    player.Position,
    player.GetAimDirection(),
    getBladePhysicalRange(player),
  );
  for (const target of targets) {
    if (
      target.IsVulnerableEnemy() ||
      target.Type === EntityType.FIREPLACE ||
      target.Type === EntityType.BOMB
    ) {
      const previousHitCountToSameEntity = countOccurrencesOfState(
        player,
        target,
      );
      flog(
        `hit count ${previousHitCountToSameEntity}, UNDEFINED: ${
          countOccurrencesOfState(player, target) === undefined
        }`,
        LOG_ID,
      );
      if (
        ...
        (countOccurrencesOfState(player, target) === undefined ||
          countOccurrencesOfState(player, target) <=
            Tuneable.maxNumberOfHitsInOneSwingToSameEntity - 1)
      ) {
        // Do the damage related things once.
        doEntityDamage(
          player,
          target,
          previousHitCountToSameEntity !== undefined
            ? previousHitCountToSameEntity
            : 0,
          0,
          1,
          0,
        ...
        const tearCountSynergy = getRandomInt(
          0,
          handleTearCountSynergies(player),
        );
...
export function doEntityDamage(
  ...
  const damageValue =
    (flatDamageIncrease +
      getBladeDamage(player) *
        damageModifier *
        Tuneable.DamageModifierForHittingSameEnemy ** index) *
